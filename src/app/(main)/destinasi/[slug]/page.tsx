...
import { Clock, Globe, MapPin, Ticket, Star, ParkingCircle, UtensilsCrossed, ShieldCheck } from "lucide-react";
...
const StarRating = ({ rating, reviewCount }: { rating: number, reviewCount: number }) => (
    <div className="flex items-center gap-2">
        <div className="flex items-center">{Array.from({ length: 5 }, (_, i) => (<Star key={i} className={`h-5 w-5 ${i < Math.floor(rating) ? 'text-yellow-400 fill-yellow-400' : 'text-gray-300'}`} />))}</div>
        <span className="font-bold text-lg">{rating.toFixed(1)}</span>
        <span className="text-sm text-muted-foreground">({reviewCount} Ulasan)</span>
    </div>
);

// Peta ikon untuk fasilitas
const iconMap: { [key: string]: React.ElementType } = {
    'Area Parkir': ParkingCircle, 'Warung Makan': UtensilsCrossed, 'Keamanan': ShieldCheck,
};
...
export default async function DestinasiDetailPage({ params }: DestinasiDetailPageProps) {
  ...
  // Ambil data destinasi, kategori, foto, dan jumlah ulasan sekaligus
  const { data: destination, error } = await supabase
    .from("destinations")
    .select(`*, categories(name), destination_photos(photo_path), reviews(count)`)
    .eq("slug", slug)
    .eq("status", "published") // Hanya tampilkan yang sudah di-publish
    .single();
  ...
  // Cek apakah user sudah mem-bookmark destinasi ini
  ...
  const { data: avgRatingData } = await supabase.rpc('get_average_rating', { dest_id: destination.id });

  // Siapkan array URL gambar untuk slider
  const imageUrls = (destination.destination_photos || []).map(photo => photo.photo_path);
  // Tambahkan gambar utama destinasi sebagai gambar pertama jika ada
  ...
  return (
    <main>
      ...
      {/* Kontainer Utama dengan Margin dan Efek Timbul */}
      <div className="container mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 -mt-24 sm:-mt-32 md:-mt-48">
        <div className="relative bg-white p-6 sm:p-8 rounded-xl shadow-lg">
            ...
            <div className="md:col-span-2 space-y-8">
                <section>
                    <h1 className="text-3xl sm:text-4xl font-bold tracking-tight">{destination.name}</h1>
                    ...
                    <div className="flex flex-wrap gap-x-6 gap-y-2 mt-4 text-sm border-t pt-4">
                        <div className="flex items-center"><Clock className="w-4 h-4 mr-2" />{destination.open_time || 'N/A'}</div>
                        <div className="flex items-center"><Ticket className="w-4 h-4 mr-2" />{destination.ticket_price || 'N/A'}</div>
                ...
                <section>
                    ...
                    <div className="prose prose-sm max-w-none text-gray-700 whitespace-pre-line leading-relaxed">{destination.description || 'Deskripsi belum tersedia.'}</div>
                </section>
                    ...
                    <div className="flex flex-wrap gap-4">
                        {destination.facilities && destination.facilities.length > 0 ? (
                            destination.facilities.map(facility => {
                                ...
                                return (<Badge key={facility} variant="outline" className="py-1 px-3 text-sm"><Icon className="h-4 w-4 mr-2" />{facility}</Badge>)
                            })
                        ) : <p className="text-sm text-muted-foreground">Informasi fasilitas belum tersedia.</p>}
                    </div>
                </section>
                ...
                <section>
                    ...
                    {user ? (
                        ...
                        <div className="text-center p-6 border-2 border-dashed rounded-lg mb-6">
                            <p className="text-muted-foreground">Anda harus <Link href="/login" className="font-semibold text-blue-600 underline">login</Link> untuk memberikan ulasan.</p>
                        </div>
                    )}
            ...
            <aside className="md:col-span-1 space-y-6 sticky top-24 h-fit">
                <Card className="shadow-none border">
                    ...
                    <CardContent className="space-y-3">
                      <BookmarkButton 
                        ...
                        isLoggedIn={!!user}
                      />
                      <Button asChild className="w-full"><a href={`https://www.google.com/maps/search/?api=1&query=${destination.lat},${destination.lng}`} target="_blank" rel="noopener noreferrer">Petunjuk Arah</a></Button>
                ...
                {tips.length > 0 && (
                <Card className="shadow-none border">
                    <CardHeader><CardTitle>Tips Berkunjung</CardTitle></CardHeader>
                    <CardContent>
